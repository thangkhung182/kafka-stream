package com.trungnguyen.kafkastream.webserver;


import com.google.gson.Gson;
import com.trungnguyen.kafkastream.model.CustomerTransactions;
...
//                kafkaStreams.store(store, QueryableStoreTypes.keyValueStore());
        var readOnlyKeyValueStore = kafkaStreams.store(
                StoreQueryParameters.fromNameAndType(store, QueryableStoreTypes.<String, Long>keyValueStore()));
...
//        ReadOnlySessionStore<String, CustomerTransactions> readOnlySessionStore =
//                kafkaStreams.store(store, QueryableStoreTypes.sessionStore());
        var readOnlySessionStore = kafkaStreams.store(
                StoreQueryParameters.fromNameAndType(
                     store,
                     QueryableStoreTypes.<String, CustomerTransactions>sessionStore()
                ));
...
//                kafkaStreams.store(store, QueryableStoreTypes.windowStore());
        var readOnlyWindowStore = kafkaStreams.store(StoreQueryParameters.fromNameAndType(
                store,
                QueryableStoreTypes.<String, Integer>windowStore()
        ));

        try (WindowStoreIterator<Integer> iterator = readOnlyWindowStore.
                fetch(key, Instant.ofEpochMilli(from), Instant.ofEpochMilli(to))) {
    ...
    private HostInfo getHostInfo(String storeName, String key) {
        var metadata = kafkaStreams.queryMetadataForKey(storeName, key, stringSerializer);
